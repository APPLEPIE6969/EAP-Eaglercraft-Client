#!/bin/bash
# ============================================================================
# EaglerCraft 26.2 - Initialize Workspace
# ============================================================================
# Decompile Minecraft 26.2 and apply EaglerCraft patches.
# 
# IMPORTANT: No MCP needed! MC 26.2 ships unobfuscated.
...
#
# Options:
#   --jar <path>         Path to minecraft-26.2.jar
...
# Show help
show_help() {
    echo "EaglerCraft 26.2 - Initialize Workspace"
    echo ""
    echo "Usage: $0 [options]"
    echo ""
    echo "Options:"
    echo "  --jar <path>         Path to minecraft-26.2.jar"
    ...
    echo "  1. Locate Minecraft 26.2 JAR (unobfuscated!)"
    echo "  2. Extract and decompile with Vineflower"
    echo "  3. Apply EaglerCraft patches"
    echo "  4. Process assets from the JAR"
    echo ""
    echo "No MCP (Mod Coder Pack) is required since MC 26.2 is unobfuscated."
}
...
main() {
    ...
    echo ""
    echo "╔══════════════════════════════════════════════════════════╗"
    echo "║   EaglerCraft 26.2 - Workspace Initialization        ║"
...
fallback_init() {
    ...
    if [ -z "$jar_path" ]; then
        info "Searching for Minecraft 26.2 JAR..."
        
        # Check common locations
        local mc_dirs=(
            "$HOME/.minecraft/versions/26.2"
            "$HOME/Library/Application Support/minecraft/versions/26.2"
            "$APPDATA/.minecraft/versions/26.2"
        )
        
        for dir in "${mc_dirs[@]}"; do
            if [ -f "$dir/26.2.jar" ]; then
                jar_path="$dir/26.2.jar"
        ...
        if [ -z "$jar_path" ]; then
            error "Could not locate Minecraft 26.2 JAR!"
    ...
    echo "version=26.2" >> "$SCRIPT_DIR/.eaglercraft-init"
