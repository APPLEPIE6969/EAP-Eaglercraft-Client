# Migration Guide: EaglerCraft 1.8 → EaglercraftX 26.2

> A comprehensive guide for migrating projects, patches, and knowledge from EaglerCraft 1.8 to EaglercraftX 26.2.

This document covers everything that changed between Minecraft 1.8 and 26.2, and how to adapt your EaglerCraft projects accordingly.
...
- [What Changed Between MC 1.8 and 26.2](#what-changed-between-mc-18-and-2612)
...
Minecraft 26.2 represents over a decade of evolution from 1.8. The codebase has been fundamentally restructured in several key areas:

| Area | MC 1.8 | MC 26.2 | Migration Difficulty |
|------|--------|-----------|---------------------|
...
## What Changed Between MC 1.8 and 26.2

### Deobfuscation Elimination

The most impactful change: **Minecraft 26.2 ships unobfuscated**. This means:
...
**Migration impact:** Your 1.8 patches reference MCP names (like `field_12345_a`). These must be translated to the actual 26.2 source names. Since 26.2 is unobfuscated, the source names are human-readable.
...
| 26.2 | Java 25 | Records, sealed classes, pattern matching, virtual threads, string templates |

Make sure your development environment uses JDK 25 or later.

### Block State System

MC 1.8 used integer metadata (0-15) for block variants. MC 26.2 uses property-based block states:
...
Since 1.8 patches reference MCP deobfuscated names and 26.2 uses the original source names, you need a name mapping:

```
1.8 MCP Name        →  26.2 Source Name
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
1.8 patches used a custom EaglerCraft patch format. 26.2 supports both unified diff and ECR:
...
Many classes were renamed, moved, or restructured between 1.8 and 26.2:

| 1.8 Class | 26.2 Class | Notes |
|-----------|-------------|-------|
...
- Add new stubs for APIs used by 26.2 that aren't in TeaVM's classlib
...
| Aspect | Protocol 47 (1.8) | Protocol 775 (26.2) |
|--------|-------------------|----------------------|
...
MC 26.2 adds a **Configuration Phase** between Login and Play:
...
1.8 used per-packet zlib compression (threshold at 256 bytes). 26.2 uses `permessage-deflate` at the WebSocket frame level:
...
**26.2 (WebSocket permessage-deflate):**
```javascript
...
The most fundamental rendering change: 1.8 used OpenGL fixed-function pipeline (`glBegin`/`glEnd`, `glVertex3f`, etc.), while 26.2 uses shader programs exclusively.
...
### EaglerCraft 1.8 vs 26.2 Adapter
...
In EaglercraftX 26.2, `EaglerAdapterGL30` maps shader-based rendering to WebGL2:

```java
// 26.2: Shader-based → WebGL2 mapping
...
26.2 adds a deferred rendering pipeline that doesn't exist in 1.8:
...
MC 1.8 used NBT (Named Binary Tag) for all item data. MC 26.2 replaces this with **Data Components**, a typed, validated, and more efficient system.
...
### 26.2: Data Component Items

```java
// Creating an enchanted diamond sword in 26.2
ItemStack sword = new ItemStack(Items.DIAMOND_SWORD);
...
| Aspect | NBT (1.8) | Data Components (26.2) |
|--------|-----------|--------------------------|
...
// 26.2: Read enchantment from data component
...
MC 1.8 chunks used a simple array-based format. MC 26.2 uses **paletted containers** for much better memory efficiency:
...
| File/Dir | 1.8 | 26.2 | Notes |
|----------|-----|--------|-------|
| `level.dat` | NBT | NBT (same format) | More fields in 26.2 |
| `region/*.mca` | Anvil | Anvil (enhanced) | Same basic format, new chunk encoding |
| `data/*.dat` | NBT | NBT | More data files |
| `entities/*.mca` | N/A | Entity region files | 26.2 stores entities separately |
| `datapacks/` | N/A | Data packs | 26.2 supports data packs natively |
...
// 26.2: Same class, but now mandatory for all registry objects
...
**Cause:** Many classes were renamed or moved between 1.8 and 26.2.

**Solution:** Use the class mapping table in this guide. Search the 26.2 source for the class's functionality rather than its 1.8 name.

### Issue: Patch fails to apply

**Cause:** The source code has changed significantly; line numbers and context no longer match.

**Solution:** Re-create the patch against the 26.2 source rather than trying to port the 1.8 patch directly. The unobfuscated source makes this much easier.
...
**Solution:** Use the MC 26.2 world upgrade mechanism. Worlds from 1.8 are automatically upgraded when first loaded in 26.2. However, in EaglerCraft, you must implement the upgrade path yourself or import worlds through the vanilla server first.
...
**Cause:** MC 26.2 uses Java 25 features not supported by older TeaVM versions.

**Solution:** Use the TeaVM version included in the EaglerCraft repository. It has been patched to support all Java 25 language features used by MC 26.2.
